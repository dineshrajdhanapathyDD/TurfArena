              ...
              {(['overview', 'matches', 'achievements'] as const).map(t => (
                <button
                  ...
                  className={`relative px-4 py-2.5 text-sm font-medium capitalize transition-colors ${
                    tab === t ? 'text-green-700' : 'text-muted-foreground hover:text-foreground'
                  }`}
                >
                  {t}
                  {tab === t && (
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
          ...
          {tab === 'overview' && (
            ...
            >
                ...
                {overviewStats.map((stat, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="bg-surface-2 rounded-2xl p-4 sm:p-5 border border-border shadow-sm"
                    >
              ...
              {/* Monthly Chart */}
              <div className="bg-surface-2 rounded-2xl p-5 border border-border shadow-sm">
                <h3 className="font-bold text-foreground mb-4">Monthly Activity</h3>
              ...
              {/* Sport Breakdown */}
              <div className="bg-surface-2 rounded-2xl p-5 border border-border shadow-sm">
                <h3 className="font-bold text-foreground mb-4">Sport Breakdown</h3>
                <div className="space-y-4">
                  {sportBreakdown.map(s => (
                    <div key={s.sport}>
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-sm font-medium text-foreground">{s.sport}</span>
                        <span className="text-xs text-muted-foreground">{s.wins}W / {s.matches - s.wins}L · {s.pct}%</span>
                      </div>
          ...
          {tab === 'matches' && (
            <motion.div
              ...
              className="space-y-3"
            >
              <h3 className="font-bold text-foreground mb-2">Recent Matches</h3>
              {recentMatches.map((match, idx) => (
                <motion.div
                  ...
                  className="bg-surface-2 rounded-xl p-4 border border-border shadow-sm flex items-center gap-4"
                >
                  ...
                  <div className="text-right">
                    <p className="font-bold text-sm text-foreground">{match.score}</p>
          ...
          {tab === 'achievements' && (
            <motion.div
              ...
              className="space-y-3"
            >
              <h3 className="font-bold text-foreground mb-2">Achievements</h3>
              {achievements.map((a, idx) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: idx * 0.06 }}
                  className={`bg-surface-2 rounded-xl p-4 border shadow-sm flex items-center gap-4 ${
                    a.done ? 'border-green-200' : 'border-border'
                  }`}
                >
                  ...
                  <div className="flex-1">
                    ...
                    {!a.done && a.progress !== undefined && a.total !== undefined && (
                      <div className="mt-2">
                        ...
                        <p className="text-[11px] text-muted-foreground mt-1">{a.progress}/{a.total}</p>
                      </div>
                    )}
                  </div>
                  {a.done && (
